...
//		LOGE("[link] audio data size: %d\n", para0);
//		uint8_t *data = (uint8_t *) para1;
//		LOGE("[link] LYLINK_AUDIO_DATA len:%d, 0x%x 0x%x 0x%x 0x%x 0x%x", para0, data[0], data[1], data[2], data[3], data[4]);
		Mutex::Autolock _l(_s_audio_info.lock);
...
#if ENABLE_LINK_START_HANDELER
void start_lylink() {
	_handler.remove_messages(E_START_LYLINK);
	_handler.send_message(E_START_LYLINK);
}
void stop_lylink() {
	_handler.remove_messages(E_STOP_LYLINK);
	_handler.send_message(E_STOP_LYLINK);
}
void restart_lylink() {
	_handler.remove_messages(E_RESTART_LYLINK);
