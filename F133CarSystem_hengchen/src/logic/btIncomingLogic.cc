...
static void _bt_call_cb(bt_call_state_e state) {
	if (state == E_BT_CALL_STATE_INCOMING) {        // 去电
...
static void onUI_init(){
	...
	mnumberTextViewPtr->setText(bt::get_call_num());
//	mnameTextViewPtr->setText(bt::get_call_contact());
	(strcmp(bt::get_call_contact().c_str(), "") == 0) ? mnameTextViewPtr->setTextTr("Unknown incoming call") : mnameTextViewPtr->setText(bt::get_call_contact());
	_bt_add_cb();
 ...
 *            触摸事件将继续传递到控件上
 */
static bool onbtIncomingActivityTouchEvent(const MotionEvent &ev) {
