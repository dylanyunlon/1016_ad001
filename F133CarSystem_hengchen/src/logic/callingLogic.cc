...
//	    mnameTextViewPtr->setText(name);
	    mnumberTextViewPtr->setText(num);
	    moutWindowPtr->setVisible(state == E_BT_CALL_STATE_OUTGOING);
	    return;
//	    LOGD("--%d-- --%s-- call_name:%s  call_num:%s \n", __LINE__, __FILE__, name.c_str(), num.c_str());
	} else if (state == E_BT_CALL_STATE_INCOMING) {
//		if (bt::get_call_contact() == "") {
//		    mnameTextViewPtr->setTextTr("Unknown incoming call");
//		} else {
//			mnameTextViewPtr->setText(bt::get_call_contact());
//		}
	    mnumberTextViewPtr->setText(bt::get_call_num());
	    minWindowPtr->setVisible(state == E_BT_CALL_STATE_INCOMING);
	    return;
	}
    minWindowPtr->setVisible(state != E_BT_CALL_STATE_TALKING);
	moutWindowPtr->setVisible(state != E_BT_CALL_STATE_TALKING);
...
//    LOGD("--%d-- --%s-- call_name:%s  call_num:%s \n", __LINE__, __FILE__, name.c_str(), num.c_str());
    ...
    minWindowPtr->setVisible(call_state == E_BT_CALL_STATE_INCOMING);
    moutWindowPtr->setVisible(call_state == E_BT_CALL_STATE_OUTGOING);
