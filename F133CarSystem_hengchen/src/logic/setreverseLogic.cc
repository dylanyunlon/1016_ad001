...
static c_cam_format_tab cam_format_tab[] = {
		...
		"/dev/video0",960,576,"CVBS PAL 50",50,
		"/dev/video0",1920,1080,"AHD 1080P 25",25,
		"/dev/video0",960,480,"CVBS NTSC 60",60,
//		"/dev/video4",720,576,"CVBS IN N/P",30,
...
static void onListItemClick_caminfoListView(ZKListView *pListView, int index, int id) {
	...
	// 保存设置格式AHD还是CVBS
	sys::setting::set_camera_format(cam_format_tab[index].format);
	// 限制1080图层显示为屏幕大小
	if (cam_format_tab[index].wide >= 1920 || cam_format_tab[index].high >=1080) {
		sys::setting::set_camera_resize(ScreenHelper::getScreenWidth(), ScreenHelper::getScreenHeight());
	} else {
		sys::setting::set_camera_resize(0, 0);
	}

	// 解决快速倒车画面卡顿的问题
	// 快速倒车和应用倒车数值不一样分开设置,
	sys::setting::set_camera_mem_type(2);

	if(strcmp(cam_format_tab[index].dev.c_str(),"/dev/video4") == 0) {
		sys::setting::set_camera_di(0);
		sys::setting::set_camera_buf_count(5);
	} else {
		if(cam_format_tab[index].format.find("CVBS") != std::string::npos) {
			sys::setting::set_camera_di(1);
			sys::setting::set_camera_buf_count(5);
		} else {
			sys::setting::set_camera_buf_count(3);
			sys::setting::set_camera_di(0);
		}
	}
}
