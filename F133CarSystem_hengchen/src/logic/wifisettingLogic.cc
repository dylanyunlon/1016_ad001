...
public:
	...
	virtual void handleWifiScanResult(std::vector<WifiInfo>* wifiInfos) {
		if (wifiInfos) {
			Mutex::Autolock _l(sLock);
	        // 新增去重逻辑
	        std::vector<WifiInfo> uniqueWifiInfos;  // 存储去重后的列表
	        std::unordered_set<std::string> seenBssids;  // 跟踪已处理的 BSSID

	        for (auto& wifi : *wifiInfos) {
	            // 假设 WifiInfo 类有 bssid 成员变量
	            const std::string& bssid = wifi.getBssid();

	            // 仅添加未处理过的 BSSID
	            if (seenBssids.find(bssid) == seenBssids.end()) {
	                uniqueWifiInfos.push_back(wifi);
	                seenBssids.insert(bssid);
	            }
	        }
	        // 使用去重后的列表
	        sWifiInfos = std::move(uniqueWifiInfos);  // 移动语义提高效率
...
static void _net_mode_cb(net_mode_e mode) {
	switch (mode) {
...
//	LOGD("[wifi] freq = %d , wifi-name = %s , wifi-mac = %s", wi.getFreq(), wi.getSsid().c_str(), wi.getBssid().c_str());
}
