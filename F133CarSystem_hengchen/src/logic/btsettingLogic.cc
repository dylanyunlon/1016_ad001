...
static void _init_bt_info() {
	menableButtonPtr->setSelected(bt::is_on());
	mconnDevTipsTextViewPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mdevNameTipsTextViewPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mbtSwitchTextViewPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mmatchTextViewPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mTextView3Ptr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mTextView2Ptr->setLongMode(ZKTextView::E_LONG_MODE_DOTS);
	mTextView4Ptr->setLongMode(ZKTextView::E_LONG_MODE_DOTS);
	_update_dev_name();
	_update_pin();
}
...
static bool onButtonClick_enableButton(ZKButton *pButton) {
    ...
    if (bt::is_on()) {
    	bt::power_off();
    	sys::setting::set_bt_power_key(false);
    	mbtEnableWindowPtr->hideWnd();		// 处理mcubt关闭比较慢
    } else {
    	sys::setting::set_bt_power_key(true);
    	bt::power_on();
    }
    return false;
}
