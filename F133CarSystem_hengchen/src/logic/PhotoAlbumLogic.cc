...
#define IMAGES_WIDTH_MAX				1280									//图片宽度限制
#define IMAGES_HEIGHT_MAX				1280									//图片高度限制
...
static bool _is_carousel_pic = false;
...
static bool _have_mem_free(int memfree, Photos pts) {
	return ((memfree-(pts.width*pts.height*4)/1024) > 4096);
}
...
static Photos ImagesFilter(std::string path) {
	...
	int MemFree = 0;
	if(!_have_mem_free(GetMemFree(), pts)) {
		fy::drop_caches();
	}
	MemFree = GetMemFree();
...
static void UpdateStatus(){
	...
	mUSB1Ptr->setSelected(albumtype==E_STORAGE_TYPE_USB2);
	mUSB2Ptr->setSelected(albumtype==E_STORAGE_TYPE_USB1);
}
 ...
 * @选择图片
 */
static void _select_picture_index(int index){
	IndexPic = index;
	...
	Photos photo;
	photo.path = "";
	photo.Displayable = true;

	sprintf(indextext,"%d/%d",IndexPic+1,size);
//	mIndexPtr->setText(indextext);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Pre_Wnd_Index()], photo);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Curr_Wnd_Index()],ImagesFilter(media::get_image_file(albumtype,IndexPic)));
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Next_Wnd_Index()], photo);
}

static void _next_picture(){
	if(PAsw.isRunning()) {
		LOGD("Can't _next_picture : Is Switching");
		return;
	}
	ZKTextView *PhotoFrame[] = {mIndexPic1Ptr,mIndexPic2Ptr,mIndexPic3Ptr};
	int size = media::get_image_list_size(albumtype);
//	IndexPic += (IndexPic < (size - 1))?1:(1 - size);
	IndexPic = (IndexPic + 1) % size;
	char indextext[24] = {0};
	Photos photo;
	photo.path = "";
	photo.Displayable = true;
...
//	mIndexPtr->setText(indextext);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Pre_Wnd_Index()], photo);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Curr_Wnd_Index()], photo);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Next_Wnd_Index()],ImagesFilter(media::get_image_file(albumtype,IndexPic)));
}

static void _pre_picture() {
	if(PAsw.isRunning()) {
		LOGD("Can't _pre_picture : Is Switching");
		return;
	}
	ZKTextView *PhotoFrame[] = {mIndexPic1Ptr,mIndexPic2Ptr,mIndexPic3Ptr};
	int size = media::get_image_list_size(albumtype);
	IndexPic -= (IndexPic > 0)?1:(1 - size);
	char indextext[24] = {0};
	Photos photo;
	photo.path = "";
	photo.Displayable = true;
	...
	sprintf(indextext, "%d/%d", IndexPic + 1, size);
//	mIndexPtr->setText(indextext);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Pre_Wnd_Index()],ImagesFilter(media::get_image_file(albumtype,IndexPic)));
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Curr_Wnd_Index()], photo);
	Horizontal_Vertical_Center_Pic(PhotoFrame[PAsw.Get_Next_Wnd_Index()], photo);
}
//旋转同步图标
...
static void onUI_show() {
	...
	if(_is_carousel_pic) {
		onButtonClick_CarouselPic(mCarouselPicPtr);
	}
}
...
//	}
//	break;
	case DELAY_DISP_PIC_TIMER:
		if (!mAlbumClassificationPtr->isWndShow()) {
			_select_picture_index(IndexPic);
		}
		return false;
		default:
			break;
...
//	mIndexPtr->setVisible(!mIndexPtr->isVisible());
	mCarouselPicPtr->setVisible(!mCarouselPicPtr->isVisible());
	mamplifyBtnPtr->setVisible(mCarouselPicPtr->isVisible());
	mreduceBtnPtr->setVisible(mCarouselPicPtr->isVisible());
	mTextView5Ptr->setVisible(mCarouselPicPtr->isVisible());
	mbackPtr->setVisible(mCarouselPicPtr->isVisible());
	if (mCarouselPicPtr->isVisible()) {
		app::show_topbar();
		sys::setting::set_reverse_topbar_show(true);
	} else {
		app::hide_topbar();
	...
	if (SlideEnable) {
		mPrevPtr->setVisible(mCarouselPicPtr->isVisible());
		mNextPtr->setVisible(mCarouselPicPtr->isVisible());
	}
}
